...
namespace Lizard.Logic.Magic
{
    public static unsafe class MagicBitboards
    {
        ...
        static MagicBitboards()
        {
            ...
            MagicSquare* FancyRookMagics   = InitializeFancyMagics(Piece.Rook, RookTable);
            MagicSquare* FancyBishopMagics = InitializeFancyMagics(Piece.Bishop, BishopTable);
            ...
            if (UsePext)
            {
                ...
                NativeMemory.AlignedFree(RookMagics);
                NativeMemory.AlignedFree(BishopMagics);

                RookMagics = FancyRookMagics;
                BishopMagics = FancyBishopMagics;
            }
            else
            {
                ...
                //  InitializeFancyMagics borked everything in RookTable and BishopTable
                //  so we need to free + reinitialize these.
                NativeMemory.AlignedFree(RookMagics);
                NativeMemory.AlignedFree(BishopMagics);
                RookMagics   = InitializeMagics(Piece.Rook, RookTable);
                BishopMagics = InitializeMagics(Piece.Bishop, BishopTable);
        ...
        public static ulong GetRookMoves(ulong boardAll, int idx)
        {
            ...
            return (UsePext) ? m.attacks[pext(boardAll, m.mask)] :
        ...
        private static ulong TestPextFancy(ulong boardAll, int idx)
        {
            ...
            ref MagicSquare rm = ref RookMagics[idx];
        ...
        /// </summary>
        private static MagicSquare* InitializeFancyMagics(int pt, ulong* table)
        {
            MagicSquare* magicArray = AlignedAllocZeroed<MagicSquare>(64);
            ...
            for (int sq = A1; sq <= H8; sq++)
            {
                ...
                {
                    m.attacks = magicArray[sq - 1].attacks + iter;
                }
                ...
                do
                {
                    m.attacks[pext(combo, m.mask)] = SlidingAttacks(pt, sq, combo);
                    iter++;
        ...
        private static MagicSquare* InitializeMagics(int pt, ulong* table)
        {
            var bits   = (pt == Piece.Bishop) ? BishopBits : RookBits;
            var magics = (pt == Piece.Bishop) ? BishopMagicNumbers : RookMagicNumbers;
            ...
            for (int sq = A1; sq <= H8; sq++)
            {
                ...
                do
                {
                    m.attacks[(combo * m.number) >> m.shift] = SlidingAttacks(pt, sq, combo);
                    combo = (combo - m.mask) & m.mask;
                    iter++;
                } while (combo != 0);
